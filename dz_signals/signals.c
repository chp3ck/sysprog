...
int sigCatcher(void){
	printf("-----------------------------------------------------\n\rCatcher starts (settings for signals interception)...\n\r");
        if (signal(SIGHUP, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGHUP(1)!!!\n\r");
        }
        if (signal(SIGINT, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGINT(2)!!!\n\r");
        }
        if (signal(SIGQUIT, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGQUIT(3)!!!\n\r");
        }
        if (signal(SIGILL, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGILL(4)!!!\n\r");
        }
        if (signal(SIGTRAP, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGTRAP(5)!!!\n\r");
        }
        if (signal(SIGABRT, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGABRT(6)!!!\n\r");
        }
        if (signal(SIGBUS, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGBUS(7)!!!\n\r");
        }
        if (signal(SIGFPE, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGFPE(8)!!!\n\r");
        }
        if (signal(SIGKILL, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGKILL(9) (by design in linux)!!!\n\r");
        }
        if (signal(SIGUSR1, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGUSR1(10)!!!\n\r");
        }
        if (signal(SIGSEGV, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGSEGV(11)!!!\n\r");
        }
        if (signal(SIGUSR2, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGUSR2(12)!!!\n\r");
        }
        ...
        if (signal(SIGTERM, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGTERM(15)!!!\n\r");
        }
        if (signal(SIGSTKFLT, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGSTKFLT(16)!!!\n\r");
        }
        if (signal(SIGCHLD, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGCHLD(17)!!!\n\r");
        }
        if (signal(SIGCONT, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGCONT(18)!!!\n\r");
        }
        if (signal(SIGSTOP, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGSTOP(19) (by design in linux)!!!\n\r");
        }
        if (signal(SIGTSTP, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGTSTP(20)!!!\n\r");
        }
        if (signal(SIGTTIN, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGTTIN(21)!!!\n\r");
        }
        if (signal(SIGTTOU, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGTTOU(22)!!!\n\r");
        }
        if (signal(SIGURG, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGURG(23)!!!\n\r");
        }
        if (signal(SIGXCPU, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGXCPU(24)!!!\n\r");
        }
        if (signal(SIGXFSZ, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGXFSZ(25)!!!\n\r");
        }
        if (signal(SIGVTALRM, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGVTALRM(26)!!!\n\r");
        }
        if (signal(SIGPROF, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGPROF(27)!!!\n\r");
        }
        if (signal(SIGWINCH, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGWINCH(28)!!!\n\r");
        }
        if (signal(SIGIO, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGIO(29)!!!\n\r");
        }
        if (signal(SIGPWR, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGPWR(30)!!!\n\r");
        }
        if (signal(SIGSYS, sigHandler) == SIG_ERR){
                printf("Can't catch the signal SIGSYS(31)!!!\n\r");
        }
	printf("Catcher ends...\n\r---------------\n\r");
        return 0;
}
